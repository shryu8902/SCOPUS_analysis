...
#%%
journal_list = pd.read_csv('Journal_ISSN.csv') #저널 리스트 읽어오기
journal_meta = {} # 저널별 데이터 취합을 위한 dict 생성

for index, info in tqdm(journal_list.iterrows()):
    h5_name = './Scopus_Meta_h5/'+info.ISSN+'.h5' #각 저널의 h5 파일 불러오기
    temp=[]    #temp 리스트에 연도별 데이터 프레임을 하나씩 저장
    for year in range(2000,2021): #
        df = pd.read_hdf(h5_name,key = str(year))
        if ~df.empty:
            temp.append(df)
    journal_meta[index] = pd.concat([temp[i] for i in range(len(temp))])     #temp 리스트 내 저장된 데이터 프레임 모두 합쳐서 저널별로 저장하기
    journal_meta[index].issn = info.ISSN
    journal_meta[index].publicationName = info.Journal
ALL_meta = pd.concat([journal_meta[i] for i in range(len(journal_list))],ignore_index=True) # 모든 저널 데이터를 하나의 데이터 프레임으로 합치기
ALL_meta = ALL_meta[~ALL_meta.creator.isnull()] #Null 정보 데이터 삭제
ALL_meta = ALL_meta[~ALL_meta.description.isnull()]
ALL_meta.reset_index(drop=True,inplace =True) 
JEL_keyword = ALL_meta.authkeywords.str.contains('J[eE][lL]\sclassification|Classification\s[LNKSE][.]',case = True,na=False) # 문헌분석에 불필요한 JEL 키워드 삭제                 
ALL_meta.authkeywords[JEL_keyword]=None
ALL_meta.coverDate = pd.to_datetime(ALL_meta.coverDate)
#%%
# 다음 파트에서 검색하기 위한 키워드를 정의함.
...
#%%
# 각 키워드가 논문 제목, 초록, 키워드에 포함되어있는지 여부에 대한 벡터
...
#%%
# Query 포함된 논문을 CSV로 저장함.
ALL_meta[Query].to_csv('ALL_meta_core.csv') # JEL classification keyword 제거
